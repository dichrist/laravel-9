...
@section('content')
    <h1 class="text-2x1 font-semibold leading-tigh py-2">Listagem dos usuários <a href="{{route('users.create')}}" class="bg-blue-900 rounded-full text-white px-4 text-sm">+</a></h1>
    ...
    <table class="min-w-full leading-normal shadow-md rounded-lg overflow-hidden">
        ...
        @foreach ($users as $user)
            <tr>
                ...
                <td class="px-5 py-5 border-b border-gray-200 bg-white text-sm">
                  <a href="{{ route('comments.index', $user->id) }}" class="bg-blue-200 rounded-full py-2 px-6">Comentários ({{ $user->comments->count() }})</a>
              </td>
            </tr>
        @endforeach
        </tbody>
